define(["jquery", "./d3.min", "./nv.d3.min", "css!./nv.d3.min.css"],
    function ($, d3, nv) {
        'use strict';
...
function drawStreamChart($element, layout, fullMatrix, self) {
    ...
    $element.empty().append($('<div />').attr({
        "id": id,
        "class": "qv-object-nvd3-stacked-area"
    }).css({
        height: height,
        width: width
    }));
...
var viz = function (self, data, labels, width, height, id, selections, layout, $element) {
    ...
    var dataNVD3 = data.map(function (row) {
        return {
            "key": row[0].qText,
            "x": convertToUnixTime(row[1].qNum),
            "y": row[2].qNum
        };
    });
    // Transform data set
    dataNVD3 = d3.nest()
        .key(function (d) {
            ...
            return listKey.indexOf(a) - listKey.indexOf(b);
        })
        .entries(dataNVD3)
        .map(function (k) {
            return {
                "key": k.key,
                "values": k.values.map(function (v) {
                    return [v.x, v.y]
                })
            }
        });
...
function assignDefaultValues(dates, dataset, defaultValue) {
    ...
    $.each(dataset, function (index1, setObject) {
        ...
        newData.push({
            key: setObject.key,
            seriesIndex: setObject.seriesIndex,
            values: newValues
        });
    });
    return newData;
